...
bool RollNum::init()
{
	...
	m_pTexture = TextureCache::getInstance()->getTextureForKey(fullpath);
    SpriteFrame *pFrame = SpriteFrame::createWithTexture(m_pTexture, Rect(0, 0, NUMBERWIDTH, NUMBERHEIGHT));
...
void RollNum::updateNumber(float dt)
{
    ...
    if(m_nCurTexH >= 180)
        h = 180;
    SpriteFrame *pFrame = SpriteFrame::createWithTexture(m_pTexture, CCRectMake(0, h, NUMBERWIDTH, NUMBERHEIGHT));
...
//	Texture2D *pTex = TextureCache::getInstance()->addImage("number.png");
//    m_pBatchNode = SpriteBatchNode::createWithTexture(pTex);
